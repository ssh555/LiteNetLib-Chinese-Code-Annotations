...
{
    /// <summary>
    /// NetManager.Socket用于网络数据的发送|接收
    /// </summary>
    public partial class NetManager
    {
        /// <summary>
        /// 轮询套接字以检查是否有可读数据时的超时时间 -> 即指定时间内是否有可用的数据可以读取
        /// </summary>
        ...
        /// <summary>
        /// IOCTL 控制码，用于禁用 UDP 在特定情况下的连接复位行为。这是 Windows 上的一个特殊配置，通常用于避免 UDP 连接由于收到 ICMP 错误消息而自动重置
        /// </summary>
        ...
        /// <param name="bufferEndPoint">远程IP</param>
        /// <param name="maxReceive">最多接收的数据包数量</param>
        ...
        /// <summary>
        /// 从目标远程端口接收数据包 -> 调用OnMessageReceived
        /// </summary>
        /// <param name="s"></param>
        ...
        public bool Start(IPAddress addressIPv4, IPAddress addressIPv6, int port, bool manualMode)
        {
            ...
            // 自动 -> 若为手动，则需要自己手动每帧调用ManualUpdate
            // 在Unity中需要在主线程中执行，建议使用手动模式，在Unity中每帧调用ManualUpdate，但是可能会涉及帧长问题，可考虑使用FixedUpdate
            if (!manualMode)
            {
                // 线程执行的方法
